...
def _user_has_any_role(user: models.User, db: Session, role_names: List[str]) -> bool:
    ...
    If role_names is empty, treat it as no restriction.
    """
    if not role_names:
        return True
...
def require_any_role(role_names: List[str]):
    """
    Factory that returns a FastAPI dependency which checks that
    the current user has at least one of the given roles.
    Used like: Depends(require_any_role(["admin", "super_admin"]))
    """
    def dependency(
...
def require_role(role_names: List[str]):
    """
    Backwards compatible alias for older code that imports require_role.
    """
